...
struct EvalNod : public EvalNode<ValTateti, DataTateti> {
  ValTateti operator() (ValTateti v_nodo, ValTateti v_final,
                        DataTateti dat_nodo) {
...
DataTateti insert_mov(Player player, StateTateti * state) {
  ...
  while (1) {
    ...
    if ((std::cin >> i >> j) && (i >= 1 && i < 4) && (j >= 1 && j < 4)
...
int main() {
  ...
  Mcts<ValTateti, DataTateti, Nod, StateTateti> m(&sel, &exp, &sim, &ret,
                                                   &sel_res);
  std::vector<Mcts<ValTateti, DataTateti, Nod, StateTateti> *>
