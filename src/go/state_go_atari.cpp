...
unsigned int StateGo::get_liberty_block(Block * block, Block * flag, INDEX i,
                                        INDEX j, INDEX & lib_i, INDEX & lib_j) {
  Blocks[i][j] = flag;
  INDEX t_i, t_j;
...
inline bool StateGo::is_block_in_atari(INDEX i, INDEX j, INDEX & i_atari,
                                       INDEX & j_atari) {
  if (Blocks[i][j]->adj > 4)
    return false;
...
DataGo StateGo::look_for_delete_atari(Block * block, Block * flag, INDEX i,
                                      INDEX j, int &max_size) const {
  Blocks[i][j] = flag;
