...
RaveEnv::RaveEnv(Config& cfg_input, StateGo* state) {
  ...
  _sel = new SelectionUCTRave<ValGo, DataGo>(cfg_input.bandit_coeff,
                                             cfg_input.amaf_coeff);
...
void Game::debug() const {
  ...
  for (int i = _size - 1; i >= 0; i--)
    for (int j = 0; j < _size; j++)
      visits[i][j] = 0;
  if (_cfg.rave) {
    NodeUCTRave<ValGo, DataGo> *root = (NodeUCTRave<ValGo, DataGo>*)
        _mcts->get_root();
    if (!root)
      return;
    ...
    for (; iter != root->children_end(); iter++)
      if (!IS_PASS((*iter)->data)) {
        ...
        coeffs[(*iter)->data.i][(*iter)->data.j] =
            SelectionUCTRave<ValGo, DataGo>(_cfg.bandit_coeff, _cfg.amaf_coeff)
    ...
    for (; iter != root->children_end(); iter++)
      if (!IS_PASS((*iter)->data)) {
        visits[(*iter)->data.i][(*iter)->data.j] = (*iter)->get_visits();
        coeffs[(*iter)->data.i][(*iter)->data.j] =
            SelectionUCT<ValGo, DataGo>(_cfg.bandit_coeff)
...
void Game::match_patterns() const {
  ...
  for (int i = 0; i < v.size(); i++)
    std::cout << "Position: " << POS_TO_LETTER(v[i].j) << " "
         << int(1 + v[i].i) << std::endl;
