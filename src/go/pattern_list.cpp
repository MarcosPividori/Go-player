...
void PatternList::add(Pattern pattern) {
  ...
  int border_sum = 0;
  if (pattern.x[0][1] == p_border)
    border_sum += 1;
  if (pattern.x[1][0] == p_border)
    border_sum += 3;
  if (pattern.x[2][1] == p_border)
    border_sum += 2;
  if (pattern.x[1][2] == p_border)
    border_sum += 6;
  if (border_sum)
  ...
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < 3; j++)
      ...
      case p_everything:
        break;
      case p_no_white:
        pattern.x[i][j] = p_no_black;
        break;
      case p_no_black:
        pattern.x[i][j] = p_no_white;
        break;
      case p_border:
        break;
      default:
...
bool PatternList::match(StateGo * state, INDEX i, INDEX j) const {
  ...
  if (i == 0)
    border_sum += 1;
  else if (i == state->_size - 1)
    border_sum += 2;
  if (j == 0)
    border_sum += 3;
  else if (j == state->_size - 1)
    border_sum += 6;
  if (border_sum)
    sum += 19683 + border_sum * 729;
  return list[sum];
...
void PatternList::read_file(const char *file_name) {
  ...
  while (!myfile.eof()) {
    ...
    case 'b':
      ...
      break;
    case 'x':
      pat.x[i][j] = p_everything;
      k++;
      break;
    case 'W':
      pat.x[i][j] = p_no_white;
      k++;
      break;
    case 'B':
      pat.x[i][j] = p_no_black;
      k++;
      break;
    case '*':
      pat.x[i][j] = p_border;
