...
void StateGo::get_atari_escape_moves(std::vector<DataGo> &v) {
  ...
  for (int i = 0; i < atari_blocks->size(); i++) {
    ...
    if (!IS_PASS(e))
      for (int count = (Blocks[k][l]->size + size) * atari_delete_coeff;
           count > 0; count--)
    ...
    if (no_self_atari_nor_suicide(d.i, d.j, turn)) {
      ...
      FOR_EACH_ADJ(d.i, d.j, m, n, {
        if (Stones[m][n] == Empty)
          sum++;
        if (Stones[m][n] == turn && Blocks[m][n] != Blocks[k][l])
          sum = 4;
      });
      if (sum > 2)
        for (int count = (Blocks[k][l]->size * atari_escape_coeff); count > 0;
             count--)
...
void StateGo::get_capture_moves(std::vector<DataGo> &v) const {
  if (pass == 2)
    return;
  int c = 0;
  Block *block;
...
void StateGo::apply(DataGo d) {
  ...
  if (IS_PASS(d)) {
    ...
    FOR_EACH_ADJ(i, j, k, l, {
      if (Stones[k][l] == opp && Blocks[k][l] != b[0]
          && Blocks[k][l] != b[1]
          && Blocks[k][l] != b[2]
