...
class MctsParallel_GlobalMutex : public MctsParallel<Value, Data, State> {
 ...
 public:
  MctsParallel_GlobalMutex(std::vector<Mcts<Value, Data, Nod, State> *> &m,
                           State* init_state, Data init_data);
...
MctsParallel_GlobalMutex<Value, Data, Nod, State>::MctsParallel_GlobalMutex(
    std::vector<Mcts<Value, Data, Nod, State> *>& m,
    State* init_state,
...
template<class Value, class Data, class Nod, class State>
void MctsParallel_GlobalMutex<Value, Data, Nod, State>::
run_time(double time_limit) {
  std::thread *threads = new std::thread[_m.size()];
  for (int i = 0; i < _m.size(); i++)
    threads[i] = std::thread(&Mcts<Value, Data, Nod, State>::run_time, _m[i],
...
template<class Value, class Data, class Nod, class State>
void MctsParallel_GlobalMutex<Value, Data, Nod, State>::
run_cycles(unsigned long cycles_limit) {
  std::thread *threads = new std::thread[_m.size()];
  for (int i = 0; i < _m.size(); i++)
    threads[i] = std::thread(&Mcts<Value, Data, Nod, State>::run_cycles, _m[i],
...
template<class Value, class Data, class Nod, class State>
void MctsParallel_GlobalMutex<Value, Data, Nod, State>::
reinit(State* init_state, Data init_data) {
  _state = init_state;
  _root->delete_tree();
  _root = new Nod(0, init_data);
}
...
template<class Value, class Data, class Nod, class State>
MctsParallel_GlobalMutex<Value, Data, Nod, State>::
~MctsParallel_GlobalMutex() {
  _root->delete_tree();
}
...
template<class Value, class Data, class Nod, class State>
MctsParallel_Root<Value, Data, Nod, State>::
MctsParallel_Root(std::vector<Mcts<Value, Data, Nod, State> *>& m,
                  State* init_state,
...
template<class Value, class Data, class Nod, class State>
void MctsParallel_Root<Value, Data, Nod, State>::
run_cycles(unsigned long cycles_limit) {
  std::thread *threads = new std::thread[_m.size()];
  for (int i = 0; i < _m.size(); i++)
    threads[i] = std::thread(&Mcts<Value, Data, Nod, State>::run_cycles, _m[i],
