...
class NodeUCTRave {
 ...
 private:
  ...
  NodeUCTRave(Value init_value, Data init_data, NodeUCTRave<Value,
              Data> *father);
 public:
  typedef typename std::vector<NodeUCTRave<Value, Data> *>::const_iterator
      const_iterator;
  ...
  unsigned long get_visits() const {
    return visits;
  };
  double get_sqrt_visits() const {
    return sqrt_visits;
  };
  void set_amaf_visits(unsigned long v) {
    amaf_visits = v;
  };
  unsigned long get_amaf_visits() const {
    return amaf_visits;
  };
  double get_sqrt_for_amaf() const {
    return sqrt_for_amaf;
  };
  NodeUCTRave<Value, Data> *get_parent()const {
    return parent;
  };
...
template<class Value, class Data>
inline void NodeUCTRave<Value, Data>::create_child(Value init_value,
                                                    Data init_data) {
  NodeUCTRave<Value, Data> *n =
      new NodeUCTRave<Value, Data> (init_value, init_data, this);
...
void NodeUCTRave<Value, Data>::delete_tree() {
  for_each(children.begin(), children.end(),
           std::mem_fun(&NodeUCTRave<Value, Data>::delete_tree));
  delete this;
}

template<class Value, class Data>
NodeUCTRave<Value, Data> *NodeUCTRave<Value, Data>::
move_root_to_child(Data child_data) {
...
template<class Value, class Data>
void NodeUCTRave<Value, Data>::count_nodes(int level,
                                            long counter[],
                                            int max_level) const {
  if (level >= max_level)
    return;
...
void NodeUCTRave<Value, Data>::debug() const {
  std::cout << "DEBUG STATS NODE: vis=" << visits << " win=" << value
      << " vis_amaf=" << amaf_visits << " win_amaf=" << amaf_value << std::endl;
  std::cout << "                  rate=" << (value / visits)
      << "  amaf_rate=" << (amaf_value / amaf_visits)
      << "  amaf_coeff=" << (sqrt(k_rave) / sqrt_for_amaf) << std::endl;
}
