...
template<class Nod, class State>
class Expansion {
 public:
...
template<class Value, class Nod>
...
class Mcts {
 ...
 public:
  ...
  void run_time(double time_limit, Nod* root, State* init_state);
  void run_cycles(unsigned long cycles_limit, Nod* root, State* init_state);
  Data get_resultant_move(Nod* root);
  void set_mutex(std::mutex* mutex) {
    _mutex = mutex;
  };
 private:
  void run_one_cycle(Nod* root, State* init_state);
};
