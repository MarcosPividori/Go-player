...
class SelectionUCTRave : public Selection<NodeUCTRave<Value, Data>> {
 ...
 public:
#endif
  double get_uct_amaf_val(const NodeUCTRave<Value, Data> *nod,
                          double log_parent);
 public:
...
template<class Value, class Data>
SelectionUCTRave<Value, Data>::SelectionUCTRave(double coeff,
                                                 double amaf_coeff) :
  _coeff(coeff), _amaf_coeff(sqrt(amaf_coeff)) {
}

template<class Value, class Data>
inline double SelectionUCTRave<Value, Data>::
get_uct_amaf_val(const NodeUCTRave<Value, Data> *nod,
  ...
  return (((double) nod->value) / (double) nod->get_visits())* (1 -
                                                                 amaf_coeff) +
      (((double) nod->amaf_value) / (double) nod->get_amaf_visits()) *
      amaf_coeff + _coeff* sqrt_log_parent / nod->get_sqrt_visits();
}

template<class Value, class Data>
NodeUCTRave<Value, Data> *SelectionUCTRave<Value, Data>::
select(const NodeUCTRave<Value, Data> *nod) {
  typename NodeUCTRave<Value, Data>::const_iterator iter =
      nod->children_begin();
  if (iter == nod->children_end())
    return NULL;
  NodeUCTRave<Value, Data> *max_nod = *iter;
  double max_val, val, sqrt_log_parent =
      sqrt(log((double) (nod->get_visits())));
...
retro(NodeUCTRave<Value, Data>* node, Value value) {
  do {
