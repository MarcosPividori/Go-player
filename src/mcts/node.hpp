...
class NodeUCT {
 ...
 public:
  typedef typename std::vector<NodeUCT<Value, Data> *>::const_iterator
      const_iterator;
  Value value;
  Data data;
  ...
  NodeUCT<Value, Data>* get_parent() const {
    return parent;
  };
...
template<class Value, class Data>
inline void NodeUCT<Value, Data>::create_child(Value init_value,
                                                Data init_data) {
  NodeUCT<Value, Data> *n =
      new NodeUCT<Value, Data> (init_value, init_data, this);
...
void NodeUCT<Value, Data>::delete_tree() {
  for_each(children.begin(), children.end(),
           std::mem_fun(&NodeUCT<Value, Data>::delete_tree));
  delete this;
}

template<class Value, class Data>
NodeUCT<Value, Data> *NodeUCT<Value, Data>::
...
template<class Value, class Data>
void NodeUCT<Value, Data>::count_nodes(int level,
                                        long counter[],
                                        int max_level) const {
  if (level >= max_level)
    return;
