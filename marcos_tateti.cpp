...
class StateTateti: public State<ValTateti,DataTateti>{
    private:
        CELL A[3][3];
    public:
        Player turn;
...
ValTateti fun(ValTateti v_nodo,ValTateti v_final,DataTateti dat_nodo)
{
...
int main()
{
    NodeUCT<ValTateti,DataTateti> *nod=new NodeUCT<ValTateti,DataTateti>(0,0,NULL),*next;
    StateTateti state;
    DataTateti res;
    std::vector<DataTateti> v;
    
    SelectionUCT<ValTateti,DataTateti> sel(1);
    ExpansionAllChildren<ValTateti,DataTateti> exp(2,0);
    SimulationTotallyRandom<ValTateti,DataTateti> sim;
    RetropropagationSimple<ValTateti,DataTateti> ret(fun);
    SelectResMostRobust<ValTateti,DataTateti> sel_res;
    Mcts<ValTateti,DataTateti,NodeUCT<ValTateti,DataTateti>> m(&sel,&exp,&sim,&ret,&sel_res);
    ...
    while((v.clear(),state.get_possible_moves(v),!v.empty())){
        ...
        if((next=nod->move_root_to_child(res))){
            ...
            nod->delete_tree();
            nod= new NodeUCT<ValTateti,DataTateti>(0,res,NULL);
        }
